    ...
    login() {
        ...
        if (lastFailedLoginTime) {
            ...
            if (timeDifference < 20 * 60 * 1000) {
                ...
                this.toast('error', 'Login Failed', `Too many login attempts. Try again in ${remainingTime} seconds`);
        ...
        this.accountService.login(email, pin).subscribe(account => {
            ...
            if (account.verified === "Disabled") {
                this.toast('warn', 'Login failed', 'Account is disabled. Contact your administrator');
                return;
            }
            ...
            if (this.loginAttempt == 5) {
                ...
                console.error(err);
                this.toast('error', 'Login failed', 'User is not authenticated. Please try again');
            }
